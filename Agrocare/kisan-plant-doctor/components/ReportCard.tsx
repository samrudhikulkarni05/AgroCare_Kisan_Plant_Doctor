...
const ReportCard: React.FC<ReportCardProps> = ({ report, onBack }) => {
  ...
  // Robust Fallbacks
  const treatmentSteps = report.diagnosis.treatment_steps && report.diagnosis.treatment_steps.length > 0 
    ? report.diagnosis.treatment_steps 
    : ["Wait for detailed technical scan..."];
    
  const preventionTips = report.diagnosis.prevention_tips && report.diagnosis.prevention_tips.length > 0 
    ? report.diagnosis.prevention_tips 
    : ["Maintain proper spacing for airflow.", "Regularly monitor leaf undersides for pests."];
